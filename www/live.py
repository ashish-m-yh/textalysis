	...
	def post(self):
		...
		if lkey != conf.demo_key:
			if not os.path.exists(lkey_file):
				self.render("./error_output.html", errormsg="Invalid licence key entered", infomsg="", lkey="", demo_key="", tmpfile="")
			else:
				with open (lkey_file,"r") as out:
					...
					if (used >= limit):
						self.render("./error_output.html", errormsg="Licence expired", infomsg="", lkey="", demo_key="", tmpfile="")
					else:
						allowed = True		
		else:
			allowed  = True
		...
		if allowed and self.request.files.has_key('comment_file'):
			...
			if (re.search("excel|spreadsheet",finfo["content_type"])):
				...
				try:
					...
					self.render("./error_output.html", errormsg=errormsg, infomsg="", lkey="", demo_key="", tmpfile="")
			...
			if ( lkey != conf.demo_key and ((used + maxrows) > limit) and allowed):
				errormsg = "You have " + str(limit) + " allowed accesses of which you have used " + str(used) + ". You have " + str(limit-used) + " accesses remaining but your data set has " + str(len(matrix)) + " rows. Therefore, please renew your licence to process this data set."
				self.render("./error_output.html", errormsg=errormsg, infomsg="", lkey="", demo_key=conf.demo_key, tmpfile=tmpfile)
			elif allowed:
				...
				self.render("./error_output.html",errormsg="",infomsg=infomsg, lkey=lkey, demo_key=conf.demo_key, tmpfile=tmpfile)
