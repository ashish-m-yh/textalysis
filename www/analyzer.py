        ...
        try:
            ...
            if self.request.files.has_key('comment_file'):
                ...
                if (finfo['content_type'] == "text/plain"):
                    ...
                    try:
                        ...
                        self.write(
                            'Expecting a plain text or XLS (not XLSX) file. <a href="javascript: history.go(-1)">Go back</a>')
                        wrong_mime = True
            ...
            for r in rows:
                if (r.strip()):
                    ...
                    if (int(this_row[1]) >= int(pos_val) or
                            int(this_row[2]) >= int(neg_val)):
                        output.append([r, this_row[0], this_row[1], this_row[
                            2], cust_agent])

                        if (agents.has_key(cust_agent)):
                            my_ctr = agents[cust_agent][2] + 1
                            agents[cust_agent] = [
                                (int(agents[cust_agent][0]) + int(this_row[1]))
                                / my_ctr,
                                (int(agents[cust_agent][1]) + int(this_row[2]))
                                / my_ctr, my_ctr
                            ]
                        else:
                            agents[cust_agent] = [int(this_row[1]),
                                                  int(this_row[2]), 1]

#						if (agents[cust_agent][0] + agents[cust_agent][1] < 100):
#							agents[cust_agent][0] = agents[cust_agent][0] + 1

                    if (int(this_row[1]) >= int(pos_val) and
                            int(pos_val) != 0):
                        pos_ctr = pos_ctr + 1

                    if (int(this_row[2]) >= int(neg_val) and
                            int(neg_val) != 0):
                        neg_ctr = neg_ctr + 1
            ...
            self.write(
                "There was an error due to one of the following reasons - ")
            self.write("<ul><li>Your licence key has expired or invalid</li>")
            self.write(
                "<li>You uploaded a file of a wrong input format</li></ul>")
