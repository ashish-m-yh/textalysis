...
#			os.remove(rpt_file)
	...
	def get(self,filename,lkey):
		...
		try: 
			...
			self.render("./error_output.html", infomsg="Report not found.", errormsg="",lkey="")
			return None
		...
		rowcount = rowcount + 1
		sheet1.write(rowcount,0,"The rows are ordered from most negative to least negative")

		rowcount = rowcount + 1
		sheet1.write(rowcount,0,"Positive and negative highlights indicate the issues which the customer commented on")
		...
		if lkey == conf.demo_key and re.search(r'/app/demoreport',self.request.uri): 
			self.render("./demo_output.html", reporturl="/excel/" + tmpfile, negs=n_distrib, pos=p_distrib, p_total=p_total, n_total=n_total, agents=agent_tuples)
		else:
			self.render("./prod_output.html", reporturl="/excel/" + tmpfile, negs=n_distrib, pos=p_distrib, p_total=p_total, n_total=n_total, agents=agent_tuples)
